...
export const sendAdminNewOrderAlert = async (order, recipients) => {
    try {
        ...
        const mailOptions = {
            ...
            html: `
                <div style="font-family: Arial, sans-serif;">
                    ...
                    <a href="${process.env.NEXT_PUBLIC_APP_URL}/admin/orders" style="display: inline-block; background: #4f46e5; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; margin-top: 10px;">View Order</a>
                </div>
            `
        };
...
export const sendOrderStatusUpdate = async (order) => {
    try {
        ...
        await transporter.sendMail(mailOptions);
        console.log(`📧 Order Status Update sent to ${order.email}`);
    } catch (error) {
        console.error('Failed to send status update email:', error);
    }
};

export const sendLowStockAlert = async (products, recipients) => {
    try {
        ...
        const mailOptions = {
            ...
            html: `
                <div style="font-family: Arial, sans-serif;">
                    ...
                    <a href="${process.env.NEXT_PUBLIC_APP_URL}/admin/products" style="display: inline-block; background: #4f46e5; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Manage Inventory</a>
                </div>
            `
        };
